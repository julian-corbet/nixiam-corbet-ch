# modules/lldap.nix
#
# The LDAP identity directory this repo's other module (pocket-id.nix) and
# any external consumer (a mail server's LDAP-directory client, a git
# forge's LDAP auth, a Kubernetes dashboard's OIDC-via-pocket-id login --
# anything that wants one shared user/group store) ultimately authenticate
# against -- implemented today by lldap specifically (a lightweight,
# LDAP-speaking user/group store with its own small web admin UI). Named
# after the actual upstream project rather than an abstract "ldap-directory"
# role, matching this repo's convention (shared with its sibling nix* repos):
# a fake generic interface with exactly one implementation behind it
# documents a boundary that doesn't exist. If a second directory backend is
# ever added, it gets its own module.
#
...
# records inside it are different things. This repo's own pocket-id module
# is a case in point from the OTHER direction: its LDAP-sync wiring -- bind
# DN, filters, attribute mapping -- is configured entirely out-of-band,
# through pocket-id's own admin UI/API, and stored encrypted in pocket-id's
# own database. See modules/pocket-id.nix's header for the full honesty
# note on why that's a real boundary, not a gap in either module.)
#
...
let
  ...
  # `modules/core.nix` here (unlike nixnet's core+providers shape). This
  # repo's sibling module, pocket-id.nix, does NOT consume `baseDn` at all,
  # even though it ultimately talks to this same directory -- its LDAP-sync
  # wiring (bind DN, base DN, filters, attribute mapping) is entirely
  # out-of-band runtime state in pocket-id's own database (see
  # pocket-id.nix's header). If a future module DOES need to share this
  # value at eval time (a different SSO/directory client with a genuinely
  # Nix-declarable LDAP config), a small shared module (mirroring nixnet's
  ...
  # convention. Not done here because no such consumer exists in this repo
  # yet.
...
in
{
  options.nixiam.lldap = {
    ...
    domain = mkOption {
      ...
      example = "example.com";
      description = ''
        Organization domain this directory serves. lldap itself has
        ...
        its organization domain (`"example.com"` -> `"dc=example,dc=com"`
        and `"admin@example.com"`). If yours don't, or you would rather
        not derive anything from a single value, leave this unset and set
    ...
    baseDn = mkOption {
      ...
      description = ''
        ...
        DN mirrors its organization domain. Any other module in your
        configuration that talks to this directory (a mail server's LDAP
        ...
        pocket-id's admin UI, not read from this option (see that
        module's header).
      '';
    };
